...
int default_enterprise_length = sizeof(default_enterprise);
...
_sess_open(struct snmp_session *in_session)
{
    ...
    session = slp->session;
    slp->transport = snmp_transport_parse(session->peername, session->local_port, session);
            ...
            struct snmp_session *session, struct snmp_pdu *pdu)
{
    ...
    case SNMP_VERSION_2c:
        return netsnmp_community_build(pkt, pkt_len, offset, session, pdu);
    case SNMP_VERSION_3:
